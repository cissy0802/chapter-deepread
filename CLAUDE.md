...
## 2. 写作要求（重点）
- **篇幅：中文 3000–4500 字（精读版），宁精炼勿注水。** 一章覆盖面比单篇论文广，略放宽，但讲透了就收、别为凑字重复。
- **硬上限由 `.maxchars` 卡住（当前 6500）——发布前自己先量，别让闸门替你发现问题。** 计数口径 = 中文页剥掉 `<style>`/`<script>`/标签后的汉字数（科普版 + 精读版合计）。`publish.sh` 超限直接 `exit 1`、本次一篇都发不出去、白白浪费一个触发时段，所以**写完先跑一遍**：

  ```
  python3 -c 'import sys,re;h=open(sys.argv[1]).read();h=re.sub(r"<style.*?</style>","",h,flags=re.S);h=re.sub(r"<script.*?</script>","",h,flags=re.S);h=re.sub(r"<[^>]+>","",h);print(len(re.findall(r"[一-鿿]",h)))' {slug}-book{N}.html
  ```

  超了就**删最长的几段**（通常是 §4 里展开过头的子节、§6 堆了太多实证），**不要整篇重写**——重写会连带把结构和图一起改坏。
- **为什么有这条**：2026-09-05 对全仓 55 篇量过一次，**篇篇超规格**，且是一条清晰的自我模仿棘轮——基准页 `ddia-ch01` 4517 字，book 41–52 中位已到 7462，涨了 66%。每一期照着上一期写，就会每代长一点。`.maxchars` 先设在 6500 止住棘轮（不为难当前水位的稿子），**冲刺结束后要收到 5500 逼回规格**（见 §2.7）。
...
- **出口代理会拦掉很多域，别在这上面耗回合数（实测，据 2026-09-10 的运行日志）**：云端沙箱的 egress proxy 已知拦截 `www.uber.com`、`medium.com`、`en.wikipedia.org`、`redis.io`、`aws.amazon.com`、`www.elastic.co`、`www.mongodb.com`、`h3geo.org`、`s2geometry.io`、`docs.cloud.google.com`、`engineeringblog.yelp.com` 等；`cloud.google.com/blog`、`github.com`（含各项目 README）、`sre.google`、`abseil.io` 实测可达。**一个域被拦就换来源，别反复重试、别绕道找镜像**——上一次运行为此连吃 8 个 `EGRESS_BLOCKED`。拿不到原话就按硬规则处理：只留文字出处、或干脆不写，**绝不因为取不到就改用记忆里的"原话"**。
...
`TOPICS.md` 里以 **`- **合辑 · …**`** 开头的条目不是某一章，而是**把原书若干章浓缩成一页**——用于管理 / 文化 / 软技能 / 研究方法这类章：单独成篇太薄，全跳又丢内容。条目里的「覆盖 …」写明它吃掉哪几章。

和普通章的差别只有四处，其余（双模式、Glossary、配图、字数下限、xref、中英双页、发布流程）完全一致：

1. **§2「坐标」改写成**：这几章在原书里的位置，以及**为什么把它们合并**（共同主题是什么）。
2. **§4「核心内容，讲透」按章分 `<h3>`**：一章一个子节，每节留下**一条可操作的判据或做法**，别退化成章节摘要。宁可每章讲薄一点，也要每章都有一条能带走的东西。
3. **篇幅放宽到中文 4000–5500 字**（普通章是 3000–4500）。覆盖的章多，但不是章数 × 单章篇幅——提纯不是叠加。
4. **文件名 `{slug}-book{N}.html`**，slug 形如 `sre-digest01-oncall-and-incident-response`；index 的 `<span class="num">` 写 **`合辑`**（英文页 `Digest`）而不是 `Ch N`。

判据仍是那句：读完这一页，约等于读懂了这几章——不是「知道这几章讲过什么」。

## 2.7 临时优先级：Xu + Chaos 期间的降频提醒
